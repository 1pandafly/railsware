<?php
/**
 * Railsware functions and definitions
 *
...
function railsware_setup() {
	/*
		...
		* If you're building a theme based on Railsware, use a find and replace
		* to change 'railsware' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'railsware', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'railsware' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'railsware_custom_background_args',
			array(
 ...
 */
function railsware_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'railsware_content_width', 640 );
}
add_action( 'after_setup_theme', 'railsware_content_width', 0 );
 ...
 */
function railsware_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'railsware' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'railsware' ),
...
add_action( 'widgets_init', 'railsware_widgets_init' );
